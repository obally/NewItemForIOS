...
+ (RACSignal *)racPOSTWthURL:(NSString *)url params:(NSDictionary *)params {
    ...
    return [[RACSignal createSignal:^RACDisposable *(id <RACSubscriber> subscriber) {
        ...
        [manager POST:url parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            [self handleResultWithSubscriber:subscriber operation:manager responseObject:responseObject];
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            [self handleErrorResultWithSubscriber:subscriber operation:manager error:error];
        }];
        return [RACDisposable disposableWithBlock:^{
           
        }];
       
    }] setNameWithFormat:@"<%@: %p> -racPOSTWthURL: %@, params: %@", self.class, self, url, params];
}
...
+ (RACSignal *)racGETWthURL:(NSString *)url isJSON:(BOOL)isJSON {
    ...
    return [RACSignal createSignal:^RACDisposable *(id <RACSubscriber> subscriber) {
        ...
        [manager GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            [self handleErrorResultWithSubscriber:(id <RACSubscriber>) subscriber operation:manager error:error];
        }];
        return [RACDisposable disposableWithBlock:^{
...
+ (void)handleErrorResultWithSubscriber:(id <RACSubscriber>)subscriber operation:(AFHTTPSessionManager *)operation error:(NSError *)error {
...
+ (void)handleResultWithSubscriber:(id <RACSubscriber>)subscriber operation:(AFHTTPSessionManager *)operation responseObject:(id)responseObject {
    //在此根据自己应用的接口进行统一处理
